...
export const apiService = {
  ...
  async getTickets() {
    let localTickets = [];
    try {
      const stored = await AsyncStorage.getItem('@dhobix_support_tickets');
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.tickets && Array.isArray(data.tickets)) {
          ...
          await AsyncStorage.setItem('@dhobix_support_tickets', JSON.stringify(merged)).catch(() => {});
          return merged;
        }
      }
    } catch (err) {
  ...
  async createTicket(ticketData) {
    ...
    try {
      ...
      await AsyncStorage.setItem('@dhobix_support_tickets', JSON.stringify(localTickets));
    } catch (e) {}
  ...
  async updateTicketStatus(ticketId, newStatus) {
    ...
    try {
      ...
      if (stored) {
        ...
        await AsyncStorage.setItem('@dhobix_support_tickets', JSON.stringify(updated));
      }
    } catch (e) {}
  ...
  async resetPassword({ email, student_id, new_password }) {
    try {
      ...
      if (response.ok && data.success) {
        ...
        try {
          const stored = await AsyncStorage.getItem('@dhobix_registered_users');
          if (stored) {
            ...
            if (idx !== -1) {
              ...
              await AsyncStorage.setItem('@dhobix_registered_users', JSON.stringify(users));
            }
          }
        } catch (e) {}
        return data;
