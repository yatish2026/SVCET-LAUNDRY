...
export const ProfileScreen = () => {
  ...
  const userAvatarKey = useMemo(() => {
    if (profile?.id) return `@vastra_user_avatar_${profile.id}`;
    if (profile?.email) return `@vastra_user_avatar_${profile.email.trim().toLowerCase()}`;
    return null;
  ...
  const handlePickAvatar = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        await AsyncStorage.removeItem('@vastra_user_avatar').catch(() => {});
        if (Platform.OS === 'web') {
  ...
  const handleSignOut = () => {
    if (Platform.OS === 'web') {
      const confirmSignout = window.confirm('Are you sure you want to sign out from VASTRA?');
      if (confirmSignout) {
        signOut();
      }
    } else {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={styles.signOutBtnText}>Sign Out from VASTRA</Text>
      </TouchableOpacity>
